  ...
  const handleChange = async (e) => {
    ...
    if (value.length > 40) {
      ...
      if (e.target.id == "address") {
        console.log(address);
        var resp = await contract.getTokenName(address);
        console.log('respsefsefresp---->', resp);
        if (!resp?.name) {
    ...
    if (Wallet_Details?.UserAccountAddr != "") {
      ...
      if (Wallet_Details.UserAccountAddr == Wallet_Details.Admin_Address) {
        ...
        if (Object.keys(errors).length == 0) {
          ...
          console.log("contract", formData.address);
          var hash = await contract.AddTokenType([formData.name.toUpperCase()], [formData.address])
